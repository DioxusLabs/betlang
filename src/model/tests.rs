...
#[test]
fn detects_prompt_from_instruction() {
    let detection = crate::detect(
        "Write a short story about a robot learning to paint. Keep it under 500 words.",
    );
    assert_eq!(top_kind(&detection), Some(Kind::Prompt));
}
...
#[test]
fn detects_each_kind_fixture_file() {
    let mut failures = Vec::new();

    for (expected, path) in KIND_FIXTURES {
        ...
        let actual = detection.kind();

        if actual != Some(expected) {
            let top = detection
                .top_kinds()
                .map(|(probability, kind)| format!("{}:{probability:.3}", kind.slug()))
                .collect::<Vec<_>>()
                .join(", ");
            failures.push(format!(
                "{path}: expected {}, got {:?}; top [{}]",
                expected.slug(),
                actual.map(Kind::slug),
                top
            ));
        }
    }
...
#[test]
fn kind_fixtures_cover_model_kinds() {
    let fixture_kinds = KIND_FIXTURES
        .into_iter()
        .map(|(kind, _)| kind)
        .collect::<HashSet<_>>();
    let model_kinds = (0..Kind::MODEL_LABEL_COUNT)
        .map(|index| Kind::from_model_index(index).expect("model label"))
        .collect::<HashSet<_>>();

    assert_eq!(fixture_kinds, model_kinds);
}

#[test]
fn detect_accepts_non_utf8_inputs() {
    let mut bytes =
        b"Translate the following sentence into French and explain the grammar.".to_vec();
...
#[test]
fn probabilities_sum_to_one_across_model_kinds() {
    let detection =
        crate::detect("Summarize this article in two sentences for a general audience.");
...
fn runtime_inference_accepts_short_sources() {
    let source = "Explain how photosynthesis works in one paragraph.\n";
...
fn empty_input_returns_empty_detection() {
    assert!(crate::detect("").top_kinds().next().is_none());
}

#[test]
fn very_short_input_returns_empty_detection() {
    // < 8 non-whitespace bytes
    assert!(crate::detect("hi").top_kinds().next().is_none());
}

fn top_kind(detection: &crate::Detection) -> Option<Kind> {
    detection.kind()
}
...
const KIND_FIXTURES: [(Kind, &str); 12] = [
    ...
    (Kind::Prompt, "tests/fixtures/prompt/question.txt"),
    (Kind::Prompt, "tests/fixtures/prompt/rewrite.txt"),
    (Kind::Prompt, "tests/fixtures/prompt/summarize.txt"),
    (Kind::Prompt, "tests/fixtures/prompt/write-code.txt"),
];
